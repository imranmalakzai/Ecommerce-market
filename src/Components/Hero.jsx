...
<section className="w-full flex xl:flex-row flex-col justify-center xl:h-max max-container gap-10 p-2">
    ...
    <div className="relative w-full flex-1 flex justify-center items-center max-xl:py-4 bg-color  bg-cover bg-image max-sm:w-full max-sm:flex-1">
      <img src={bigShoe}  className="object-contain max-sm:w-full max-sm:flex-1 relative z-10 w-[610px] h-[500px]" alt="shoes1"/>
      <div className="flex sm:gap-6 absolute max-sm:justify-center -bottom-[5%]">
       {
        bigShoes.map((Item,index) => (
          <div key={index} className="flex justify-center items-center sm:w-40 sm:h-40 rounded-xl max-sm:p-4 pr-12 max-sm:h-[100px] max-sm:w-[100px]">
